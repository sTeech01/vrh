...
- Hash-based роутер: `#dashboard`, `#projects`, `#project/id`, `#item/projectId/itemId`
...
DOMContentLoaded → createClient → getSession
  ...
  → есть сессия → initApp()
    ...
    → loadRemoteData()   // 5 параллельных запросов:
...
```js
{
  ...
  components,    // [{id, name, quantity, unit, done}] — только Legacy
