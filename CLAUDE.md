...
js/nomenclature.js — модуль «Номенклатура»: унификация названий материалов (nom-*)
...
| `manager_tasks` | `id TEXT PK, title TEXT, description TEXT, status TEXT, priority TEXT, assignee_name TEXT, parent_id TEXT, project_id TEXT, deadline TIMESTAMPTZ, completed_at TIMESTAMPTZ, completion_comment TEXT, created_at` — задачи сотрудников |
| `manager_task_comments` | `id TEXT PK, task_id TEXT, text TEXT, author TEXT, created_at` — комментарии к задачам |
| `suppliers` | справочник контрагентов (модуль «Контрагенты», js/suppliers.js) — организации, тип, статус |
| `nomenclature_items` | `id TEXT PK, name TEXT, sort_order INT, created_at` — позиции номенклатуры (модуль «Номенклатура») |
| `nomenclature_columns` | `id TEXT PK, assignee_name TEXT, sort_order INT, created_at` — колонки-специалисты с вариантами названий |
| `nomenclature_values` | `item_id TEXT, column_id TEXT, value TEXT, updated_at` — PK(item_id,column_id) — предложение конкретного специалиста по конкретной позиции |
...
| 086–133 | Модуль «Склад»: номенклатура, категории, движение (приход/расход/инвентаризация), фиксы UI модалок, фиксы прогрочки закупок, фильтр «Ожидает», поле «Готово» у подпозиций |
| 134 | Модуль «Задачи»: `manager_tasks`, `manager_task_comments`; плоский список с вкладками, статусы, приоритеты, подзадачи-чеклист, лента комментариев, переназначение, дата завершения |
| 135 | Фикс layout модалок «Добавить/Редактировать материал»: `modal-header` + `wh-modal-body` + `wh-modal-footer` вместо inline-стилей; `wh-grid2` для строки Количество/Единица |
| 136 | Фикс ширины лейбла в карточке контакта поставщика: `56px` → `80px`, чтобы TELEGRAM/WhatsApp не выбивались за границу |
| 151–159 | Модуль «Исполнители»: справочник с ФИО/должность/тел/email, карточка профиля, поиск; быстрый выбор статуса/исполнителя в задачах; архив задач; фикс позиционирования пикера |
| 160 | Полностью новый отчёт по задачам: KPI-карточки, достижения за 7 дней, работа по исполнителям, блок проблем/блокировок, риски, загрузка сотрудников с прогресс-барами, следующие действия, авто-резюме руководителю |
| 161 | Фикс дублей исполнителей в задачах (Путилин А.А./Зюков/Парамузов) — нормализация алиасов ФИО при загрузке |
| 162 | Исполнители только из справочника «Исполнители» (убрано inline-добавление); «Поставщики» → «Контрагенты»; выбор ответственного контрагента на позиции |
| 163 | Задачи: цветной бейдж приоритета в строке, список всегда сортируется по приоритету |
| 164 | Фикс переполнения выпадающего списка контрагентов/исполнителей (max-height + scroll, защита позиционирования) |
| 165 | Цветной бейдж контрагента (по палитре исполнителей), уменьшенные пустые бейджи «Назначить»/«Контрагент»; фикс номера деплоя в сайдбаре (не обновлялся с #160) |
| 167 | Новый модуль «Номенклатура» (замена неактивной заглушки «Документы»): унификация названий материалов, колонки-специалисты с выбором исполнителя из справочника, 442 позиции из склад-номенклатуры |
| 168 | Фикс модуля «Номенклатура»: debounce поиска (поле теряло фокус после каждой буквы), уникальные id при добавлении позиций/колонок (риск коллизии при Date.now() без соли) |
| 169 | «Номенклатура»: горизонтальный + вертикальный скролл таблицы в ограниченной по высоте области (шапка зафиксирована, как в Excel), обрезка длинных названий многоточием с подсказкой по наведению |
| 170 | «Номенклатура»: строка добавления позиции перемещена под панель поиска/кнопки «Колонка», перед таблицей (была под таблицей) |
| 171 | «Номенклатура»: все элементы управления верхнего блока (поиск, «+Колонка», «Новая позиция», «Добавить позицию») выровнены по правому краю в две строки друг под другом |
| 172 | «Номенклатура»: поле «Новая позиция» и кнопка «Добавить позицию» приведены к одинаковой высоте (36px); плашка исполнителя в заголовке колонки окрашивается в его цвет из общей палитры (было одинаковым серым для всех) |
| 173 | «Номенклатура»: поле «Новая позиция» и кнопка «Добавить позицию» гарантированно на одной строке (flex-wrap:nowrap) — раньше при недостатке ширины переносились на два уровня |
| 174 | Production OS: чекбокс «Скрыть отгруженные» в панели фильтров проекта — скрывает позиции со статусом отгрузки «Отгружено» |
| 175 | Задачи: фикс перепутанного алиаса ФИО (Зюков/Парамузов нормализовались в сокращённую форму вместо полной); поле «Исполнитель» и фильтр по исполнителю переведены со свободного текста на строгий выбор из справочника «Исполнители» |
